#!/usr/bin/env python3
"""
Tool to recursively find and convert lowercase enum string literals to uppercase.
Only converts strings within quotes (single or double) and only for known M8 enum types.
"""
...
# All enum names to look for
ALL_ENUM_NAMES = INSTRUMENT_TYPES + MODULATOR_TYPES

def create_pattern():
    """Create regex pattern to match quoted enum names in lowercase."""
    # Create alternation of all names
    names_pattern = "|".join([name.lower() for name in ALL_ENUM_NAMES])
...
def process_file(file_path, pattern, dry_run=True):
    """Process a single file, converting lowercase enum names to uppercase."""
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            content = f.read()
            
        # Check if the file contains any of our targets
        if not any(name.lower() in content for name in ALL_ENUM_NAMES):
            return False, 0
            
        # Function to replace matches with uppercase version
        def replace_match(match):
            open_quote = match.group(1)
            enum_name = match.group(2).upper()
            close_quote = match.group(3)
            return f"{open_quote}{enum_name}{close_quote}"
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python convert_enum_case.py [--dry-run] directory1 [directory2 ...]")
        sys.exit(1)
