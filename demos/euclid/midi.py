...
"""
Euclid MIDI Demo - Euclidean rhythm patterns via External MIDI
...
- Traditional rhythm patterns from world music
...
def create_euclid_midi_project(kick_channel: int, snare_channel: int, hat_channel: int):
    """Create the Euclid MIDI M8 project."""
    print(f"Creating Euclid MIDI demo: {PROJECT_NAME}")
    ...
    project.metadata.directory = "/Songs/pym8-demos/euclid-midi/"
    ...
    for row in range(NUM_ROWS):
        ...
        print(f"  Kick:  E({kick_rhythm_name})")
        ...
        print(f"  Snare: E({snare_rhythm_name})")
...
def save_project(project: M8Project):
    ...
    print(f"  Patterns: Euclidean rhythms (Bjorklund algorithm)")


def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(
        description="Generate Euclidean rhythm patterns via External MIDI"
    )
    parser.add_argument(
    ...
    project = create_euclid_midi_project(
        args.kick_channel,
        args.snare_channel,
        args.hat_channel
    )
